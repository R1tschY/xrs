...
Letter = f"{BaseChar} | {Ideographic}"


def parse_num(s):
    if not re.match(r"#x[A-F0-9]{1,4}", s):
...
def parse_elem(s):
    if s.startswith("["):
        ...
        return parse_num(left), parse_num(right) + 1
    else:
        num = parse_num(s)
        return num, num + 1
...
def gen_map(s):
    bounds = [parse_elem(elem.strip()) for elem in s.split("|")]
    bounds.sort(key=itemgetter(0))

    last_right = 0
    opt_bounds = []
    for left, right in bounds:
        if last_right == left:
            opt_bounds[-1][1] = right
        else:
            opt_bounds.append([left, right])

        last_right = right

    rust_code = ", ".join([f"({left:#x}, {right:#x})" for (left, right) in opt_bounds])
    print(f"BOUNDS: {len(opt_bounds)} (optimized: {len(bounds) - len(opt_bounds)})")
    print(rust_code)
...
if __name__ == '__main__':
    ...
    gen_map(f"{Char}")
    print("== START CHAR")
    gen_map(f"{Letter} | #x{ord('_'):04X} | #x{ord(':'):04X}")
    print("== NAME CHAR")
    gen_map(f"{Letter} | {Digit} | #x{ord('.'):04X} | #x{ord('-'):04X} | #x{ord('_'):04X} | #x{ord(':'):04X} | {CombiningChar} | {Extender}")
